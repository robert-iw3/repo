...
if [ $command = "start" ]; then
  ...
  docker container exec hbase bash -c "echo \"create 'deduplication','cf'\" | hbase shell -n"
  docker container exec hbase bash -c "echo \"create 'exchange_rate','cf'\" | hbase shell -n"
  docker container exec hbase bash -c "echo \"put 'exchange_rate','ID$(date '+%Y-%m-%d')','cf:exchange_rate',Bytes.toBytes(0.00007)\" | hbase shell -n"
  docker container exec hbase bash -c "echo \"put 'exchange_rate','MY$(date '+%Y-%m-%d')','cf:exchange_rate',Bytes.toBytes(0.24)\" | hbase shell -n"
  docker container exec hbase bash -c "echo \"put 'exchange_rate','SG$(date '+%Y-%m-%d')','cf:exchange_rate',Bytes.toBytes(0.74)\" | hbase shell -n"
  docker container exec hbase bash -c "echo \"put 'exchange_rate','PH$(date '+%Y-%m-%d')','cf:exchange_rate',Bytes.toBytes(0.02)\" | hbase shell -n"
  docker container exec hbase bash -c "echo \"scan 'exchange_rate'\" | hbase shell -n"
  sleep 30
  if [ $pipeline = "flink" ]; then
    docker container exec flink_jobmanager bash -c \
      "./bin/flink run \
	    --detached \
    ...
    docker container exec spark_submit bash -c \
      "spark-submit \
      --verbose \
      --master ${SPARK_MASTER} \
      --deploy-mode cluster \
