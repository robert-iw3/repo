...
impl ParquetHandler {
    ...
    async fn process_files(&self) -> Result<()> {
        ...
        for file in files {
            ...
            if let Err(e) = self.process_file(&file).await {
                ERRORS_TOTAL.with_label_values(&[Path::new(&file).file_name().unwrap().to_str().unwrap()]).inc();
    ...
    async fn process_file(&self, file: &str) -> Result<()> {
        ...
        let file_reader = SerializedFileReader::new(std::fs::File::open(file)?)?;
        let mut arrow_reader = ArrowReader::new(file_reader);
        let schema_arrow = arrow_reader.get_schema()?;
        ...
        let mut pos = self.load_position(&conn, &file_name)?;
        ...
        for record_batch in arrow_reader {
            ...
            for row in 0..record_batch.num_rows() {
                ...
                EVENTS_PROCESSED.with_label_values(&[&file_name]).inc();
    ...
    fn validate_schema(&self, arrow_schema: &arrow::datatypes::Schema, schema: &Schema) -> Vec<String> {
    ...
    fn transform_to_cim(&self, event: &Value, schema: &Schema) -> Value {
        let mut cim = serde_json::Map::new();
        cim.insert(
            "time".to_string(),
            event
                .get(schema.mappings.cim.get("time").unwrap_or(&"timestamp".to_string()))
                .cloned()
                .unwrap_or(Value::Number(chrono::Utc::now().timestamp().into())),
        );
        cim.insert("vendor_product".to_string(), Value::String("Parquet_Connector".to_string()));
        cim.insert("schema".to_string(), Value::String(schema.name.clone()));

        for (key, value) in &schema.mappings.cim {
            if key != "sourcetype" {
                if value.starts_with('"') && value.ends_with('"') {
                    cim.insert(key.clone(), Value::String(value.trim_matches('"').to_string()));
                } else {
                    cim.insert(key.clone(), event.get(value).cloned().unwrap_or(Value::String("".to_string())));
                }
            }
        }

        let mut result = serde_json::Map::new();
        result.insert("event".to_string(), Value::Object(cim));
        result.insert(
            ...
            Value::String(
                schema
                    ...
                    .get("sourcetype")
                    .cloned()
                    .unwrap_or_else(|| format!("parquet:{}", schema.name)),
            ),
        );
        Value::Object(result)
    }
