...
EVENTS_PROCESSED = Counter("parquet_connector_events_processed_total", "Total events processed per file", ["file"])
ERRORS_TOTAL = Counter("parquet_connector_errors_total", "Total errors per file", ["file"])
PROCESSING_LATENCY = Histogram("parquet_connector_processing_latency_seconds", "Processing latency per file", ["file"])
ACTIVE_FILES = Gauge("parquet_connector_active_files", "Active Parquet files being processed")
...
class ParquetHandler:
    ...
    async def _discover_files(self) -> List[Path]:
        ...
        try:
            ...
            PROCESSING_LATENCY.labels("discover").observe((datetime.now() - start).total_seconds())
            ...
            try:
                ...
                for file in files:
                    ...
                    try:
                        ...
                        ERRORS_TOTAL.labels(file.name).inc()
    ...
    async def _process_file(self, file: Path):
        ...
        ACTIVE_FILES.set(len(self.active_files))
        ...
        async with AsyncExitStack() as stack:
            try:
                    ...
                    for fragment in dataset.get_fragments():
                        for batch_record in fragment.to_table().to_batches():
                            ...
                            for _, row in df.iterrows():
                                ...
                                EVENTS_PROCESSED.labels(file_name).inc()
                ...
                PROCESSING_LATENCY.labels(file_name).observe((datetime.now() - start).total_seconds())
            except pyarrow.ArrowInvalid as e:
                ERRORS_TOTAL.labels(file_name).inc()
    ...
    def _validate_schema(self, arrow_schema, schema: Schema) -> List[str]:
        expected_fields = set(v for k, v in schema.mappings.cim.items() if k != "sourcetype" and not v.startswith('"'))
    ...
    def _transform_to_cim(self, event: Dict, schema: Schema) -> Dict:
        cim = {
            ...
            "vendor_product": "Parquet_Connector",
            "schema": schema.name,
        }
        ...
        return {
            "event": cim,
            "sourcetype": schema.mappings.cim.get("sourcetype", f"parquet:{schema.name}")
        }