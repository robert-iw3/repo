...
EVENTS_SENT = Counter("sql_connector_events_sent_total", "Total events sent to Splunk", ["destination"])
SEND_ERRORS = Counter("sql_connector_send_errors_total", "Total send errors to Splunk", ["destination"])
...
async def send_to_splunk(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            try:
                events, table, count = await asyncio.wait_for(queue.get(), timeout=buffer_timeout)
                logger.info(f"Received {count} events from table {table}")
                batch.extend(events)
