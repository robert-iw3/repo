...
logger = setup_logging("ta_tenable_tenable_io_compliance")

class TENABLE_IO_COMPLIANCE(smi.Script):
    def __init__(self):
        super(TENABLE_IO_COMPLIANCE, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('tenable_io_compliance')
        scheme.description = 'TVM Compliance'
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        ingester.collect_events(collect_compliance_data=True)


if __name__ == '__main__':
    exit_code = TENABLE_IO_COMPLIANCE().run(sys.argv)
    sys.exit(exit_code)
