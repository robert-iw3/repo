    ...
    def message(self, signature, status=None, rids=None, level=logging.INFO, **kwargs):
        ...
        for rid_ntuple in rids:
            if len(rid_ntuple) == 3:
                ...
                self.logger.warning('Could not unpack rid_ntuple')
                message = ''
    ...
    def writeevents(self, index='summary', host=None, source=None, fext='common_action_model'):
        ...
        if self.events:
            ...
            if not fext or not re.match(r'^[\w-]+$', fext):
                self.logger.warning('Requested file extension was ignored due to invalid characters')
                fext = 'common_action_model'
            elif len(fext) > 200:
                self.logger.warning('Requested file extension was ignored due to length')
                fext = 'common_action_model'
            # header
