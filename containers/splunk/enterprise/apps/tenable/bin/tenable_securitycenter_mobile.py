...
logger = setup_logging("ta_tenable_tenable_securitycenter_mobile")

class TENABLE_SECURITYCENTER_MOBILE(smi.Script):
    def __init__(self):
        super(TENABLE_SECURITYCENTER_MOBILE, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('tenable_securitycenter_mobile')
        scheme.description = 'TSC Mobile Assets & Vulns'
    ...
    def validate_input(self, definition: smi.ValidationDefinition):
        ...
        _, account_stanza = utility.get_credentials(session_key, global_account_name)
        ...
        if tenable_account_type == "tenable_securitycenter_api_keys":
            ...
            tsc = TSC(host = host,port = port,ssl_verify=verify_ssl_certificate, proxies=proxies, adapter=adapter, access_key=sc_access_key,
                secret_key=sc_secret_key)
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        ingester = SCCollector(logger, ew, input_items[1], session_key, analysis_type="sc_mobile")
        ingester.collect_events()


if __name__ == '__main__':
    exit_code = TENABLE_SECURITYCENTER_MOBILE().run(sys.argv)
    sys.exit(exit_code)
