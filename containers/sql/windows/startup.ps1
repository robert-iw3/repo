<#
.SYNOPSIS
    Configures SQL Server 2025 in a Windows container during startup using parameters from script defaults or config.ini.
    Applies enterprise-grade performance, network, service, query store, database, advanced, tempdb, and user settings.
    - Calculates memory based on container resources
    - Optimizes TempDB with multiple files
    - Configures sp_configure options comprehensively
    - Sets model database properties for new DBs
    - Enables features like Query Store and Optimized Locking
    - Creates users, roles, and permissions
    - Handles service account changes (domain-joined containers)
    - Includes robust error handling with try-catch and logging

.DESCRIPTION
    This script runs as the container entrypoint to initialize and configure SQL Server after installation.
    Supports overrides via C:/config.ini (INI sections matching variable categories) or environment variables (e.g., SA_PASSWORD).
    Assumes SQL Server is installed via Dockerfile with temporary SA password; changes it to provided value.
    Network protocol changes and service restarts are applied if needed.
    Tails the error log to keep the container running.
    For production, test configurations thoroughly as some changes may require restarts or have workload-specific impacts.

.PARAMETER None
    No direct parameters; uses env:SA_PASSWORD, config.ini, and script defaults.

.EXAMPLE
    # In Dockerfile: CMD ["powershell", "-File", "C:/startup.ps1"]
    # With docker-compose environment: SA_PASSWORD="YourStrong!Passw0rd"

.NOTES
    Run in Windows Server 2025 container with SQL Server 2025 Developer edition.
    Requires SqlServer PowerShell module (installed via /FEATURES=Tools in setup).
    Domain service accounts need container domain-join; otherwise, skips with error log.
...
if (Test-Path "C:/config.ini") {
    try {
        $config = Parse-IniFile "C:/config.ini"
...
# Get environment variables (can override ini/script)
...
try {
    ...
    Write-Error "SqlServer module import failed: $_ (using T-SQL fallbacks)"
}
...
try {
    ...
    Write-Error "Failed to start MSSQLSERVER service: $_"
    exit 1
}

# Change SA password
try {
    Invoke-Sqlcmd -ServerInstance localhost -Username sa -Password 'TempP@ssw0rd!' -Query "ALTER LOGIN sa WITH PASSWORD='$sa_password'; ALTER LOGIN sa ENABLE;" -ErrorAction Stop
} catch {
    Write-Error "Failed to change SA password: $_"
    exit 1
}
...
RECONFIGURE;
"@
    Invoke-Sqlcmd -ServerInstance localhost -Username sa -Password $sa_password -Query $spConfigQuery -ErrorAction Stop
} catch {
...
if (Get-Command Set-SqlNetworkConfiguration -ErrorAction SilentlyContinue) {
    try {
        ...
        Write-Error "Failed to configure network protocols: $_"
    }
} else {
...
if ($SqlServiceAccount -ne "NT Authority\System" -or $AgentServiceAccount -ne "NT Authority\System") {
    try {
        ...
        Write-Error "Failed to change service accounts: $_ (ensure domain-joined if using domain accounts)"
    }
}

# Enable Query Store on model
try {
    Invoke-Sqlcmd -ServerInstance localhost -Username sa -Password $sa_password -Query @"
...
"@
    ...
    Invoke-Sqlcmd -ServerInstance localhost -Username sa -Password $sa_password -Query $modelQuery -ErrorAction Stop
} catch {
...
"@ -ErrorAction Stop
    ...
    if ($TraceFlags) {
        ...
        foreach ($flag in $flags) {
            Invoke-Sqlcmd -ServerInstance localhost -Username sa -Password $sa_password -Query "DBCC TRACEON($flag, -1);" -ErrorAction Stop
        }
    }
    Invoke-Sqlcmd -ServerInstance localhost -Username sa -Password $sa_password -Query "EXEC sp_configure 'default language', (SELECT langid FROM sys.syslanguages WHERE name = '$DefaultLanguage'); RECONFIGURE;" -ErrorAction Stop
} catch {
...
try {
    ...
    $currentTempFiles = Invoke-Sqlcmd -ServerInstance localhost -Username sa -Password $sa_password -Query "SELECT COUNT(*) AS count FROM sys.master_files WHERE database_id=2 AND type=0;" -ErrorAction Stop
...
if ($UserConfigs) {
    try {
        if ($DefaultDbName) {
            Invoke-Sqlcmd -ServerInstance localhost -Username sa -Password $sa_password -Query "IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = '$DefaultDbName') CREATE DATABASE [$DefaultDbName];" -ErrorAction Stop
        }
        ...
        foreach ($user in $users) {
            ...
            Invoke-Sqlcmd -ServerInstance localhost -Username sa -Password $sa_password -Query "IF NOT EXISTS (SELECT name FROM sys.sql_logins WHERE name = '$loginName') CREATE LOGIN [$loginName] WITH PASSWORD = '$password';" -ErrorAction Stop

            foreach ($role in $serverRoles) {
                Invoke-Sqlcmd -ServerInstance localhost -Username sa -Password $sa_password -Query "ALTER SERVER ROLE [$role] ADD MEMBER [$loginName];" -ErrorAction Stop
            }
            ...
            foreach ($dbRole in $dbRoles) {
                ...
                Invoke-Sqlcmd -ServerInstance localhost -Username sa -Password $sa_password -Query "USE [$roleDb]; IF NOT EXISTS (SELECT name FROM sys.database_principals WHERE name = '$loginName') CREATE USER [$loginName] FOR LOGIN [$loginName]; ALTER ROLE [$roleName] ADD MEMBER [$loginName];" -ErrorAction Stop
            }
            ...
            foreach ($perm in $permissions) {
                ...
                Invoke-Sqlcmd -ServerInstance localhost -Username sa -Password $sa_password -Query "USE [$permDb]; GRANT $permStmt TO [$loginName];" -ErrorAction Stop
            }
        }
    } catch {
...
try {
    Get-Content "C:\Program Files\Microsoft SQL Server\MSSQL16.MSSQLSERVER\MSSQL\Log\ERRORLOG" -Tail 1 -Wait
} catch {
    Write-Error "Failed to tail error log: $_"
    exit 1
}