locals {
  gwlb_subnet_ids   = [for k, v in aws_subnet.gwlb : v.id]
  gwlb_subnet_cidrs = [for k, v in aws_subnet.gwlb : v.cidr_block]
...
resource "aws_route_table" "igw" {
  ...
  tags = {
    Name = format("igw-%s", var.application_name)
  }
}
...
resource "aws_subnet" "gwlb" {
  ...
  tags = {
    Name = format("gwlb-%s-%s", var.application_name, each.value.availability_zone)
  }
}
...
resource "aws_route_table" "gwlb" {
  ...
  tags = {
    Name = format("gwlb-%s", var.application_name)
  }
}

resource "aws_route" "gwlb" {
