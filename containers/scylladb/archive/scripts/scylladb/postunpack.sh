...
# to /usr/bin/java. We update the symlink to point to /opt/java/bin/java. We also need to
# include a symlink in the /usr/lib/jvm/ directory for the scylla-jmx select-java script to work
ln -sf "${_root_dir}/java/bin/java" "/usr/bin/java"
mkdir -p "/usr/lib/jvm" && ln -s "${_root_dir}/java" "/usr/lib/jvm"
...
# to create a symlink
ln -s /opt/supervisor/bin/supervisord /usr/bin
ln -s /opt/supervisor/bin/supervisorctl /usr/bin