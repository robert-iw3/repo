...
def load_high_score_so_data() -> None:
    try:
        ...
        logger.error(f"Failed to load high-score SO data: {str(e)}")
        raise
...
def insert_so_data(data: dict) -> None:
    try:
        ...
        # Inline comment: Cypher query to import questions, tags, answers, users with uniqueness constraints
        import_query = """
        UNWIND $data AS q
        ...
        logger.error(f"Failed to insert SO data: {str(e)}")
        raise

# ... (rest unchanged, with try-except in render_page for completeness)
def get_tag() -> str:
    return st.text_input("Which tag questions do you want to import?", value="neo4j")

def get_pages():
    col1, col2 = st.columns(2)
    with col1:
        num_pages = st.number_input("Number of pages (250 questions per page)", step=1, min_value=1)
    with col2:
...
def render_page():
    try:
        ...
        st.error(f"Error in render_page: {str(e)}", icon="🚨")
