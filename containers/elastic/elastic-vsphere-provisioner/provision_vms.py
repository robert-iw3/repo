#!/usr/bin/env python3
"""
Provisions VMs with FULL USER CONFIG
"""
import os
import sys
import time
...
def download_iso():
    url = ISO_URLS.get(DISTRO)
    if not url:
        print(f"{DISTRO} ISO not available.")
        sys.exit(1)
    iso_name = url.split("/")[-1]
    iso_path = ISOS_DIR / iso_name
    if iso_path.exists():
        return str(iso_path)
    print(f"Downloading {DISTRO} ISO...")
...
def generate_cloud_init(vm_name, ip):
    ...
    if not template_dir.exists():
        print(f"Template for {DISTRO} not found.")
        sys.exit(1)
    ...
    user_data = user_data.replace("{{ allowed_subnet }}", os.getenv("ALLOWED_SUBNET"))
    ...
    subprocess.run(["mkisofs", "-o", str(iso_path), "-V", "cidata", "-r", "-J", str(cidata_dir)], check=True)
    return str(iso_path)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        specs = ast.literal_eval(sys.argv[1])
