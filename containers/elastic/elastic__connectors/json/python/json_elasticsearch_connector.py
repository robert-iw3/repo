                ...
                for line in f:
                    try:
                        ...
                        if not schema:
                            ...
                            continue
                        ecs_batch.append(self.transform_to_ecs(event, schema))
                        event_count += 1
...
async def sender_process(task_queue, es_index, file_positions, stop_event):
...
def worker_process(task_queue, es_index, file_positions, schemas):
    handler = JSONElasticsearchHandler(es_index, task_queue, file_positions, schemas)
    while True:
        try:
            file_path, position = task_queue.get(timeout=1)
            ecs_batch, file_path, new_position = handler.process_json_chunk(file_path, position)
...
def main():
    ...
    schemas = JSONElasticsearchHandler(None, None, None, None).load_schemas()

    sender = Process(target=asyncio.run, args=(sender_process(task_queue, ES_INDEX, file_positions, stop_event),))
    sender.start()
    ...
    print(f"Monitoring JSON logs in {JSON_LOG_DIR} with {WORKER_COUNT} workers for Elasticsearch")
