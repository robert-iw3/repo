...
class XMLConnectorsConfig(BaseModel):
    log_dir: str
    schemas_file: str
...
def deploy_docker(config, use_podman=False):
    ...
    env = os.environ.copy()
    env.update({
        'XML_LOG_DIR': config.xml_connectors.log_dir,
        'SCHEMAS_FILE': config.xml_connectors.schemas_file,
...
def deploy_kubernetes(config):
    ...
    manifests = ['xml-pvc.yaml', 'xml-splunk-deployment.yaml', 'xml-elasticsearch-deployment.yaml']
...
def deploy_ansible(config):
    ...
    subprocess.run(['ansible-playbook', 'deploy_xml_connectors.yml', '-e', f"config_file={os.path.abspath('deploy_config.yaml')}"], check=True)
...
def main():
    parser = argparse.ArgumentParser(description='Deploy XML connectors for Splunk and Elasticsearch')
    ...
    config_files = [
        ...
        'xml-pvc.yaml',
        'deploy_xml_connectors.yml'
    ]

    if args.cleanup:
        print("Cleaning up XML connectors deployment...")
    ...
    print(f"XML connectors deployed with log directory {config.xml_connectors.log_dir}")
