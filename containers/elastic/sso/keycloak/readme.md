...
openssl pkcs12 -export -name keycloak \
  -in ca.crt -inkey ca.key \
  -out keycloakkeystore.p12

keytool -importkeystore -destkeystore keycloak.keystore \
  -srckeystore keycloakkeystore.p12 -srcstoretype pkcs12
  -alias keycloak

keytool -import -alias keycloak -file ca.crt -keystore server.truststore
```
