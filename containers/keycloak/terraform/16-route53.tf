...
resource "aws_route53_record" "keycloak_heyvalemar_net" {
  zone_id = data.aws_route53_zone.route53_zone_1.zone_id
  name    = "keycloak.${data.aws_route53_zone.route53_zone_1.name}"
  type    = "A"
...
# Creates an A record in the Route 53 hosted zone
resource "aws_route53_record" "ssh_heyvalemar_net" {
  zone_id = data.aws_route53_zone.route53_zone_1.zone_id
