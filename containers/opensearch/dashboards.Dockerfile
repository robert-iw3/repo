...
ENV OSD_TRANSFORM_VIS_VERSION 3.2.0

USER root

ADD scripts/docker-uid-gid-setup.sh /usr/local/bin/docker-uid-gid-setup.sh
ADD scripts/jdk-cacerts-auto-import.sh /usr/local/bin/jdk-cacerts-auto-import.sh
...
RUN export BINARCH=$(uname -m | sed 's/x86_64/amd64/' | sed 's/aarch64/arm64/') && \
    ...
    python3 -m pip install --no-compile --no-cache-dir requests[security] && \
