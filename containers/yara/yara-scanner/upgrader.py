...
def needs_update(sig_url: str) -> bool:
    """
    Check if yara-scanner needs update
    """
    try:
...
class yara-scannerUpdater:
    """
    yara-scanner upgrader
    """
    ...
    def get_yara-scanner_zip_file_url(self) -> str:
        """
        Get latest yara-scanner zipfile download url from github api
        """
        response_info = requests.get(url=self.UPDATE_URL_yara-scanner, timeout=30)
    ...
    def download_yara-scanner(self) -> requests.models.Response:
        """
        ...
        try:
            log(
                "INFO",
                "Upgrader",
                f"Checking location of latest release {self.UPDATE_URL_yara-scanner} ...",
            )
            # Get download URL
            zip_url = self.get_yara-scanner_zip_file_url()
            if not zip_url:
                log(
                    "ERROR",
                    "Upgrader",
                    "Error downloading the yara-scanner update - check your Internet connection",
                )
                sys.exit(1)
            else:
    ...
    def make_dir_yara-scanner(self, target_file: str) -> None:
        """
        Make dirs for yara-scanner
        """
        try:
    ...
    def extract_yara-scanner(self, response_zip: requests.models.Response) -> None:
        """
        Extract yara-scanner
        """
        # Read ZIP file
        try:
            with ZipFile(BytesIO(response_zip.content)) as zip_update:
                for zip_file_path in zip_update.namelist():
                    ...
                    with zip_update.open(zip_file_path) as source:
                        ...
                        self.make_dir_yara-scanner(target_file)
    ...
    def update_yara-scanner(self) -> bool:
        """
        Update yara-scanner
        """
        try:
            response_zip = self.download_yara-scanner()
            self.extract_yara-scanner(response_zip)
        except Exception:
            if self.debug:
                trace()
            return False
...
if __name__ == "__main__":
    ...
    # Parse Arguments
    parser = argparse.ArgumentParser(description="yara-scanner upgrader")
    parser.add_argument(
        "--sigsonly",
        action="store_true",
    ...
    # Update yara-scanner
    updater = yara-scannerUpdater(args.debug, get_application_path())

    if not args.sigsonly:
        log("INFO", "Upgrader", "Updating yara-scanner ...")
        updater.update_yara-scanner()
