...
# yara-scanner modules
from lib.yara-scanner_args import parser
from lib.yara-scanner_logger import yara-scannerLogger, get_syslog_timestamp
from lib.yara-scanner_helpers import (
    yara-scanner_generate_hashes,
    yara-scanner_get_excluded_mountpoints,
    yara-scanner_print_progress,
    yara-scanner_transform_os,
    yara-scanner_replace_env_vars,
    yara-scanner_get_file_type,
    yara-scanner_remove_non_ascii_drop,
    yara-scanner_get_age_string,
)

from lib.yara-scanner_constants import (
    EVIL_EXTENSIONS,
    SCRIPT_EXTENSIONS,
    SCRIPT_TYPES,
    HASH_WHITELIST,
...
from lib.yara-scanner_venv import venv_check
...
class yara-scanner:
    ...
    def __init__(self, intense_mode: bool) -> None:
        ...
        if not os.path.exists(sig_dir) or os.listdir(sig_dir) == []:
            ...
            update_yara-scanner(sigs_only=True)
        ...
        if not args.force:
            ...
            if args.alldrives:
                ...
                self.start_excludes = (
                    ...
                    | set(["/media", "/volumes"])
                    | set(yara-scanner_get_excluded_mountpoints())
                )
    ...
    def scan_path(self, path):
        ...
        if os.path.isfile(path):
            ...
            args.progress = False
            yara-scanner.scan_path_files(root, directories, files)
            return
        ...
        for root, directories, files in os.walk(
            ...
            if args.multicore:
                proc = Process(
                    target=yara-scanner.scan_path_files,
                    args=(
                        root,
                        directories,
                        files,
                ...
                yara-scanner.scan_path_files(root, directories, files, progress_bar)
    ...
    def perform_intense_check(
        ...
        first_bytes_string = "%s / %s" % (
            file_data[:20].hex(),
            yara-scanner_remove_non_ascii_drop(file_data[:20]),
        )
        ...
        md5, sha1, sha256 = yara-scanner_generate_hashes(file_data)
        md5_num = int(md5, 16)
    ...
    def scan_path_files(self, root, directories, files, progress_bar=None):
        ...
        for filename in files:
            try:
                ...
                # Evaluate Type
                file_type = yara-scanner_get_file_type(
                ...
                file_info = (
                    ...
                    f" {yara-scanner_get_age_string(file_path)} "
                )
                    ...
                    with codecs.open(
                        ...
                        for line in lines:
                            try:
                                ...
                                regex = yara-scanner_replace_env_vars(regex)

                                # OS specific transforms
                                regex = yara-scanner_transform_os(regex)

                                # If false positive definition exists
                                regex_fp_comp = None
                                if "regex_fp" in locals():
                                    # Replacements
                                    regex_fp = yara-scanner_replace_env_vars(regex_fp)
                                    regex_fp = yara-scanner_transform_os(regex_fp)
    ...
    def initialize_yara_rules(self) -> None:
        ...
        try:
            ...
            # Add as yara-scanner YARA rules
    ...
    def handle_client(self, client_socket, address):
        ...
        while True:
            try:
                ...
                logger.log(
                    "NOTICE", "Results", f"Finished yara-scanner Scan CLIENT: {clientid}"
                )
                client_socket.close()
                return False
            except socket.error:
...
def update_yara-scanner(sigs_only: bool) -> None:
    """
    update yara-scanner
    """
    ...
    if args.d is True:
        if os.path.exists(args.pidfile):
            with open(args.pidfile, "r", encoding="utf-8") as fpid:
                yara-scanner_pid = int(fpid.read())
                fpid.close()
                if psutil.pid_exists(yara-scanner_pid):
                    print("yara-scanner daemon already running. Returning to Asgard.")
                    sys.exit(0)
...
def sigint_handler(signal_name: int, frame: Optional[FrameType]) -> None:
    ...
    try:
        logger.log(
            "INFO",
            "Init",
            "yara-scanner's work has been interrupted by a human. Returning to Asgard.",
        )
    except Exception:
        print("yara-scanner's work has been interrupted by a human. Returning to Asgard.")
    remove_pidfile()
    if args.debug:
...
def sigterm_handler(signal_name: int, frame: Optional[FrameType]) -> None:
    ...
    """
    remove_pidfile()
    print("yara-scanner's work has been interrupted by a SIGTERM. Returning to Asgard.")
    if args.debug:
        print(signal_name, frame)
    sys.exit(0)
...
def print_start_info() -> None:
    ...
    logger.log(
        "NOTICE",
        "Init",
        f"Starting yara-scanner VERSION: {logger.version} SYSTEM: {os.uname().nodename}"
...
def read_args() -> "argparse.Namespace":
    ...
    filename = f"yara-scanner_{os.uname().nodename}_{date_format}.log"
...
if __name__ == "__main__":
    ...
    # yara-scanner
    yara-scanner = yara-scanner(args.intense)
    ...
    if not args.noprocscan:
        if os.geteuid() == 0:
            yara-scanner.scan_processes_linux()
        else:
            logger.log(
                "NOTICE",
                "Init",
    ...
    # File scan mode
    if args.d:
        yara-scanner.run_daemon()

    if not args.d and not args.nofilescan:
        yara-scanner.scan_path(args.p)
