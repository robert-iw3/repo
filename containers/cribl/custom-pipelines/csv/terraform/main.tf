...
variable "csv_dir" { default = "./csv_files" }
variable "file_filter" { default = "*.csv" }
variable "delimiter" { default = "," }
variable "has_header" { default = true }
variable "tracking_field" { default = "modtime" }
variable "pipeline_id" { default = "my_csv_pipeline" }
variable "pipeline_group" { default = "local" }
variable "source_tag" { default = "csv_files" }
...
# Validations... (similar to previous)
...
locals {
  csv_files = fileset(var.csv_dir, var.file_filter)
}

resource "cribl_collector" "csv_collector" {
  for_each    = local.csv_files
  id          = "csv_collector_${replace(each.value, ".csv", "")}"
  type        = "file"
  description = "CSV File Collector for ${each.value} with incremental loads"
  config = {
    path               = var.csv_dir
...
output "collector_ids" {
  value = [for c in cribl_collector.csv_collector : c.id]
}

resource "null_resource" "log_creation" {
  provisioner "local-exec" {
    command = "echo '$(date): Created collectors: ${join(", ", values(cribl_collector.csv_collector)[*].id)}' >> terraform_log.txt"
  }
  depends_on = [cribl_collector.csv_collector]
}