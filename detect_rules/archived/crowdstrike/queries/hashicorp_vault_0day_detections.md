...
event_platform=Vault event_type=auth_failure request_path=/auth/userpass/login/* error!=null
| regex(request_path, "auth/userpass/login/([^/]+)", username)
| time_bucket(10m)
| group by time_bucket, src_ip
| aggregate distinct_user_count=distinct(username), attempted_users=collect(username), failed_logins=count(), avg_duration_ms=avg(response_duration_ms), stdev_duration_ms=stdev(response_duration_ms)
| filter distinct_user_count > 15 AND failed_logins > 20
| project-away time_bucket
```
...
event_platform=Vault event_type=auth_failure (request_path=/auth/userpass/login/* OR request_path=/auth/ldap/login/*) error!=null
| regex(request_path, "auth/(?:userpass|ldap)/login/([^/]+)", attempted_username)
| eval normalized_user=lower(trim(attempted_username))
| time_bucket(15m)
| group by time_bucket, src_ip, normalized_user
| aggregate distinct_variations=distinct(attempted_username), failed_logins=count(), attempted_variations=collect(attempted_username)
| filter distinct_variations > 3 AND failed_logins > 10
| project-away time_bucket
```
...
event_platform=Vault (request_path=/mfa/validate OR auth_method_type=totp)
| regex(request_body, "passcode[\'\"]\s*:\s*[\'\"]([^\'\"]+)", passcode)
| eval numeric_passcode=replace(passcode, "\s", "")
| filter numeric_passcode!=null AND length(numeric_passcode)>0
| time_bucket(5m)
| group by time_bucket, src_ip, entity_id, numeric_passcode
| aggregate distinct_passcode_variations=distinct(passcode), attempted_passcodes=collect(passcode), attempt_count=count()
| filter distinct_passcode_variations > 1
| project-away time_bucket
```
...
event_platform=Vault request_path=/auth/cert/login auth_token_type=service
| filter auth_metadata_cert_name!=null AND auth_alias_name!=null AND auth_metadata_cert_name!=auth_alias_name
| project time, src_ip, cert_role_name=auth_metadata_cert_name, impersonated_cn_alias=auth_alias_name, entity_id=auth_entity_id
```
...
event_platform=Vault request_path=/identity/entity/id/* request_method=POST
| mvexpand assigned_policy=request_data_policies
| eval normalized_policy=lower(trim(assigned_policy))
| filter normalized_policy=="root" AND assigned_policy!="root"
| project time, src_ip=request_remote_address, user=auth_display_name, target_entity_path=request_path, assigned_policy
```
...
event_platform=Vault request_path=/sys/audit/* (request_method=PUT OR request_method=POST) request_data_type=file request_data_options_mode!=null
| eval mode=request_data_options_mode, mode_len=length(mode), owner_perm=substring(mode, mode_len-2, 1), group_perm=substring(mode, mode_len-1, 1), other_perm=substring(mode, mode_len, 1)
| filter owner_perm IN ("1","3","5","7") OR group_perm IN ("1","3","5","7") OR other_perm IN ("1","3","5","7")
| project time, src_ip=request_remote_address, user=auth_display_name, target_audit_device=request_path, audit_file_path=request_data_options_file_path, file_mode=request_data_options_mode
```