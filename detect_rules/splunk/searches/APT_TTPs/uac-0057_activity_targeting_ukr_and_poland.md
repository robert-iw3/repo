...
(index=* sourcetype=xmlwineventlog:microsoft-windows-sysmon/operational OR sourcetype=stream:http OR sourcetype=pan:traffic OR sourcetype=zscaler)
| where (
    `comment("File Creation by Hash IOC")`
    (EventCode=11 AND sha256 IN ("f6fec3722a8c98c29c5de10969b8f70962dbb47ba53dcbcd4a3bbc63996d258d", "deaa3f807de097c3bfff37a41e97af5091b2df0e3a6d01a11a206732f9c6e49c", "aac430127c438224ec61a6c02ea59eb3308eb54297daac985a7b26a75485e55f", "06380c593d122fc4987e9d4559a9573a74803455809e89dd04d476870a427cbe", "082877e6f8b28f6cf96d3498067b0c404351847444ebc9b886054f96d85d55d4", "082903a8bec2b0ef7c7df3e75871e70c996edcca70802d100c7f68414811c804", "69636ddc0b263c93f10b00000c230434febbd49ecdddf5af6448449ea3a85175", "a2a2f0281eed6ec758130d2f2b2b5d4f578ac90605f7e16a07428316c9f6424e", "8a057d88a391a89489697634580e43dbb14ef8ab1720cb9971acc418b1a43564", "707a24070bd99ba545a4b8bab6a056500763a1ce7289305654eaa3132c7cbd36", "5fa19aa32776b6ab45a99a851746fbe189f7a668daf82f3965225c1a2f8b9d36", "3b5980c758bd61abaa4422692620104a81eefbf151361a1d8afe8e89bf38579d", "c7e44bba26c9a57d8d0fa64a140d58f89d42fd95638b8e09bc0d2020424b640e", "7c77d1ba7046a4b47aec8ec0f2a5f55c73073a026793ca986af22bbf38dc948c", "559ee2fad8d16ecaa7be398022aa7aa1adbd8f8f882a34d934be9f90f6dcb90b"))
    OR
    `comment("File Creation by Path IOC")`
    (EventCode=11 AND match(TargetFilename, "(?i)(\\Temp\\DefenderProtectionScope\.log|\\Microsoft\\System\\ProtectedCertSystem\.dll|\\Serv\\0x00bac729fe\.log|\\Microsoft\\Windows\\Protection overview\.lnk|\\Temp\\sdw9gobh0n|\\Microsoft\\Windows\\Protection overview past\.lnk|\\Logs\\sdw9gobh0n\.log|\\SDXHelp\\SDXHelp\.dll|\\Runtime\\RuntimeBroker\.dll|\\MSDE\\mrasp86\.dll|\\DiagnosticComponents\\DiagnosticComponents\.dll|\\ProgramData\\OfficeRuntimeBroker\.xlam|\\ProgramData\\OfficeRuntimeBroker\.lnk|\\ProgramData\\~OfficeRuntimeBroker\.dat|\\ProgramData\\ssh\\ssh\.pif\.pif\.pif|\\ProgramData\\~DF20BC61C6277A354A\.dat)$"))
    OR
    `comment("Persistence via Run Key")`
    (EventCode=13 AND match(TargetObject, "(?i)SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run$") AND Details IN ("SytemProtectionService", "MicrosoftDefender", "SytemProtectService", "Audio Driver"))
    OR
    `comment("Suspicious Rundll32 Execution")`
    (EventCode=1 AND match(Image, "(?i)\\rundll32\.exe$") AND (match(CommandLine, "(?i),#1$") OR match(CommandLine, "(?i),TS_STATUS_INFO_get0_status$") OR match(CommandLine, "(?i)shell32\.dll,ShellExec_RunDLL")))
    OR
    `comment("Suspicious Expand.exe Execution")`
    (EventCode=1 AND match(ParentImage, "(?i)\\excel\.exe$") AND match(Image, "(?i)\\expand\.exe$") AND match(CommandLine, "(?i)\.xlam") AND match(CommandLine, "(?i)\.dat") AND match(CommandLine, "(?i)ProgramData"))
    OR
    `comment("Network C2 Domain IOC")`
    ((EventCode=3 OR sourcetype IN ("stream:http", "pan:traffic", "zscaler")) AND (QueryName IN ("sweetgeorgiayarns.online", "kitchengardenseeds.icu", "punandjokes.icu", "taskandpurpose.icu", "medpagetoday.icu", "pesthacks.icu", "curseforge.icu") OR dest IN ("sweetgeorgiayarns.online", "kitchengardenseeds.icu", "punandjokes.icu", "taskandpurpose.icu", "medpagetoday.icu", "pesthacks.icu", "curseforge.icu")))
    OR
    `comment("Network C2 User-Agent IOC")`
    (http_user_agent IN ("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.80 Safari/537.36", "Mozilla/5.0 (iPhone; CPU iPhone OS 18_4_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/133.0.6943.84 Mobile/15E148 Safari/604.1") AND dest IN ("sweetgeorgiayarns.online", "kitchengardenseeds.icu", "punandjokes.icu", "taskandpurpose.icu", "medpagetoday.icu", "pesthacks.icu", "curseforge.icu"))
    OR
    `comment("Network C2 via Slack Webhook")`
    (match(url, "hooks\.slack\.com/services/") AND NOT (Image IN ("*\\slack.exe", "*\\chrome.exe", "*\\firefox.exe", "*\\msedge.exe", "*\\iexplore.exe") OR process_name IN ("slack.exe", "chrome.exe", "firefox.exe", "msedge.exe", "iexplore.exe")))
)
`comment("Normalize host and user fields and identify the detection reason")`
| eval host=coalesce(ComputerName, dest, dvc_host, host), user=coalesce(User, user, src_user)
| eval detection_reason=case(
    EventCode=11 AND isnotnull(sha256), "File Creation by Hash: " + sha256,
    EventCode=11 AND isnotnull(TargetFilename), "File Creation by Path: " + TargetFilename,
    EventCode=13, "Persistence via Run Key: " + TargetObject + " -> " + Details,
    EventCode=1 AND match(Image, "(?i)rundll32.exe$"), "Suspicious Rundll32 Execution: " + CommandLine,
    EventCode=1 AND match(Image, "(?i)expand.exe$"), "Suspicious Expand.exe from Excel: " + CommandLine,
    isnotnull(QueryName) OR isnotnull(dest), "Network C2 Communication to: " + coalesce(QueryName, dest),
    isnotnull(url), "Network C2 via URL: " + url,
    1=1, "Unknown UAC-0057 Activity"
)
`comment("Aggregate results for alerting")`
| stats count min(_time) as firstTime max(_time) as lastTime values(detection_reason) as detection_reasons by host, user
| convert ctime(firstTime) ctime(lastTime)
| rename host as impacted_host, user as impacted_user
```