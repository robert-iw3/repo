...
SELECT EventTime AS _time, EndpointName AS host, UserName AS user,
  CASE
    WHEN SrcProcCmdLine LIKE '% -enc UwB0AGEAcgB0AC0AUwBsAGUAZQBwACAALQBzACAAMQAwAA==%' THEN 'WhisperGate PowerShell'
    WHEN SrcProcCmdLine LIKE '%Set-MpPreference%ExclusionPath%' THEN 'PowerShell Defender Exclusion'
    WHEN SrcProcName = 'AdvancedRun.exe' THEN 'AdvancedRun Defender Disabling'
    WHEN SrcProcName = 'InstallUtil.exe' AND (SrcProcImagePath LIKE '%\\AppData\\Local\\Temp\\%' OR SrcProcImagePath LIKE '%\\Windows\\Temp\\%') THEN 'InstallUtil from Temp Path'
    WHEN SrcProcCmdLine LIKE '%secretsdump.py%' OR SrcProcCmdLine LIKE '%psexec.py%' THEN 'Impacket Execution'
    WHEN SrcProcName LIKE 'rclone%' AND SrcProcCmdLine LIKE '%mega%.nz%' THEN 'Rclone Exfil to MEGA'
    WHEN SrcProcName = 'java.exe' AND (SrcProcCmdLine LIKE '%-L%socks5://%' OR SrcProcCmdLine LIKE '%-L%rtcp://%') THEN 'GOST Tunneling'
    WHEN SrcProcCmdLine LIKE '%proxychains%' THEN 'ProxyChains Usage'
    WHEN SrcProcCmdLine LIKE '%su-bruteforce%' THEN 'su-bruteforce Usage'
    WHEN SrcProcCmdLine LIKE '%linpeas.%' THEN 'LinPEAS Execution'
    WHEN MD5 = '896e0f54fc67d72d94b40d7885f10c51' THEN 'GOST Tool File Hash'
    WHEN NetworkDestIP IS NOT NULL OR NetworkDestDomain IS NOT NULL THEN 'C2 Network Connection'
    WHEN DnsRequestDomainName IS NOT NULL THEN 'Iodine DNS Tunneling'
  END AS TTP_Category,
  SrcProcName AS Process_Name, SrcProcCmdLine AS CommandLine, SrcProcImagePath AS Image, MD5 AS md5, NetworkDestIP AS dest, NetworkUrl AS url, DnsRequestDomainName AS QueryName
FROM (
  /* Process Execution TTPs */
  SELECT EndpointName, UserName, EventTime, SrcProcName, SrcProcCmdLine, SrcProcImagePath, NULL AS MD5, NULL AS NetworkDestIP, NULL AS NetworkDestDomain, NULL AS NetworkUrl, NULL AS DnsRequestDomainName
  FROM deep_visibility
  WHERE EventType = 'Process Start' AND (
    (SrcProcName IN ('powershell.exe', 'pwsh.exe') AND SrcProcCmdLine LIKE '% -enc UwB0AGEAcgB0AC0AUwBsAGUAZQBwACAALQBzACAAMQAwAA==%') OR
    (SrcProcName IN ('powershell.exe', 'pwsh.exe') AND SrcProcCmdLine LIKE '%Set-MpPreference%ExclusionPath%C:\\%') OR
    (SrcProcName = 'AdvancedRun.exe' AND (SrcProcCmdLine LIKE '%stop WinDefend%' OR SrcProcCmdLine LIKE '%rmdir %C:\\ProgramData\\Microsoft\\Windows Defender%')) OR
    (SrcProcName = 'InstallUtil.exe' AND (SrcProcImagePath LIKE '%\\AppData\\Local\\Temp\\%' OR SrcProcImagePath LIKE '%\\Windows\\Temp\\%')) OR
    (SrcProcCmdLine LIKE '%secretsdump.py%' OR SrcProcCmdLine LIKE '%psexec.py%') OR
    (SrcProcName LIKE 'rclone%' AND SrcProcCmdLine LIKE '%mega%.nz%') OR
    (SrcProcName = 'java.exe' AND (SrcProcCmdLine LIKE '%-L%socks5://%' OR SrcProcCmdLine LIKE '%-L%rtcp://%')) OR
    SrcProcCmdLine LIKE '%proxychains%' OR
    SrcProcCmdLine LIKE '%su-bruteforce%' OR
    (SrcProcCmdLine LIKE '%linpeas.sh%' OR SrcProcCmdLine LIKE '%linpeas.py%')
  )
  UNION
  /* File Creation TTPs */
  SELECT EndpointName, UserName, EventTime, NULL, NULL, NULL, MD5, NULL, NULL, NULL, NULL
  FROM deep_visibility
  WHERE EventType = 'File Creation' AND MD5 = '896e0f54fc67d72d94b40d7885f10c51'
  UNION
  /* Network Connection TTPs */
  SELECT EndpointName, UserName, EventTime, NULL, NULL, NULL, NULL, NetworkDestIP, NetworkDestDomain, NetworkUrl, NULL
  FROM deep_visibility
  WHERE EventType = 'Network Connect' AND (
    NetworkDestIP IN ('5.226.139.66', '45.141.87.11', '46.101.242.222', '62.173.140.223', '79.124.8.66', '90.131.156.107', '112.51.253.153', '112.132.218.45', '154.21.20.82', '179.43.133.202', '179.43.142.42', '179.43.162.55', '179.43.175.38', '179.43.175.108', '179.43.176.60', '179.43.187.47', '179.43.189.218', '185.245.84.227', '185.245.85.251', '194.26.29.84', '194.26.29.95', '194.26.29.98', '194.26.29.251') OR
    NetworkDestDomain IN ('interlinks.top', '3proxy.ru', 'nssm.cc') OR
    (NetworkDestDomain LIKE '%cdn.discordapp.com' AND NetworkUrl LIKE '%/attachments/%') OR
    NetworkDestDomain LIKE '%ngrok.com'
  )
  UNION
  /* DNS Lookup TTPs */
  SELECT EndpointName, UserName, EventTime, NULL, NULL, NULL, NULL, NULL, NULL, NULL, DnsRequestDomainName
  FROM deep_visibility
  WHERE EventType = 'DNS Query' AND DnsRequestDomainName = 'dns.test658324901domain.me'
) AS ttps
WHERE TTP_Category IS NOT NULL
ORDER BY _time DESC
LIMIT 1000
```