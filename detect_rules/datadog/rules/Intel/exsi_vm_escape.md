...
```sql
# Multi-condition OR logic covering the different stages of the ESXi VM Escape toolkit attack chain
...
# Layer 2 – Disabling VMware VMCI/related devices via devcon.exe (Windows)
@proc.name:devcon.exe AND @proc.command_line:(
  "*disable*PCI\\VEN_15AD&DEV_0740*" OR
  "*disable*ROOT\\VMWVMCIHOSTDEV*"
)

OR

# Layer 3 – Very suspicious – KDU (Bring Your Own Vulnerable Driver) tool
@proc.name:kdu.exe

OR

# Layer 4 – Aggressive host-level outbound firewall block via netsh (Windows)
@proc.name:netsh.exe AND @proc.command_line:"*advfirewall*add*rule*action=block*dir=out*remoteip=0.0.0.0-255.255.255.255*"

OR

# Layer 5 – Modification/creation of critical ESXi inetd.conf backdoor file (mostly Linux/ESXi hosts)
@file.path:"/var/run/inetd.conf" AND @file.action:(created OR modified OR write OR change)
```