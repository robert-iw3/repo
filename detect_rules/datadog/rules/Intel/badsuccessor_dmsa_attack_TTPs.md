...
---
```sql
source:wineventlog_security event.code:5137 object.class:msDS-DelegatedManagedServiceAccount -user:(*admin* svc_* *_svc *$)
| groupby _time, host, user count, values(object.dn) as created_object_dn
```

### dMSA Attribute Modification
---
```sql
source:wineventlog_security event.code:5136 object.class:msDS-DelegatedManagedServiceAccount attribute.ldap_display_name:msDS-ManagedAccountPrecededByLink -user:(*admin* svc_* *_svc *$)
| groupby _time, host, user count, values(object.dn) as modified_object_dn, values(attribute.value) as superseded_account_dn
```

### dMSA State Change
---
```sql
source:wineventlog_security event.code:5136 object.class:msDS-DelegatedManagedServiceAccount attribute.ldap_display_name:msDS-DelegatedMSAState attribute.value:2 -user:(*admin* svc_* *_svc *$)
| groupby _time, host, user count, values(object.dn) as modified_object_dn
```

### KDS Root Key Creation
---
```sql
source:wineventlog_security event.code:5137 object.class:msKds-ProvRootKey
| groupby _time, host, user count, values(object.dn) as kds_root_key_dn
```

### Identify Kerberos TGT requests
---
```sql
source:directory_service event.code:2946 account.name:*$ KERB-DMSA-KEY-PACKAGE
| groupby account.name, client.address, host count
```