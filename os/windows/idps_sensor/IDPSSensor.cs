/********************************************************************************
 * SYSTEM:          IDPS Sensor - Bidirectional Volumetric Network Defense
 * COMPONENT:       IDPSSensor.cs (Unmanaged ETW Engine & FFI Bridge)
 * AUTHOR:          Robert Weber
 * VERSION:         1.0
 * * DESCRIPTION:
 ...
 * to execute the Rust ML engine (idpssensor_ml.dll) directly in memory, bypassing
 * IPC pipe latency.
 * * ARCHITECTURAL FEATURES:
 * - Native FFI Memory Map: Bypasses all IPC pipelines for zero-latency ML evaluation.
 * - Bidirectional Volumetric Routing: Accurately classifies Ingress, Egress, and Lateral network flows.
...
public class RealTimeIDPSSensor {
    ...
    [DllImport("idpssensor_ml.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
    private static extern IntPtr init_engine();

    [DllImport("idpssensor_ml.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
    private static extern IntPtr evaluate_telemetry(IntPtr engine, string jsonPayload);

    [DllImport("idpssensor_ml.dll", CallingConvention = CallingConvention.Cdecl)]
    ...
    public static ConcurrentQueue<IDPSEvent> EventQueue = new ConcurrentQueue<IDPSEvent>();
    ...
    public static string EvaluateBatch(string jsonPayload) {
        ...
        try {
            ...
            EventQueue.Enqueue(new IDPSEvent { Provider = "DiagLog", Message = $"FFI MARSHAL ERROR: {ex.Message}" });
            return "{}";
        } finally {
        ...
        string[] suspiciousPaths)
    {
        ...
        SetDllDirectory(@"C:\ProgramData\IDPSSensor\Bin");

        try {
            _mlEnginePtr = init_engine();
            if (_mlEnginePtr != IntPtr.Zero) {
                EventQueue.Enqueue(new IDPSEvent { Provider = "DiagLog", Message = "[ML ENGINE] Native DLL successfully mapped at memory address: 0x" });
            } else {
                EventQueue.Enqueue(new IDPSEvent { Provider = "DiagLog", Message = "[ML ENGINE ERROR] init_engine returned NULL." });
            }
        } catch (Exception ex) {
            EventQueue.Enqueue(new IDPSEvent { Provider = "DiagLog", Message = "[ML ENGINE ERROR] FFI Import Failed: " + EscapeJson(ex.Message) });
        }
        ...
        if (maliciousIps != null && maliciousDomains != null) {
            ...
            EventQueue.Enqueue(new IDPSEvent { Provider = "DiagLog", Message = $"[THREAT INTEL] Binary Search arrays loaded: {CompiledIps.Length} IPs, {CompiledDomains.Length} Domains." });
        }
    ...
    public static void StartSession() {
        Task.Run(() => {
            try {
                if (TraceEventSession.GetActiveSessionNames().Contains("IDPSRealTimeSession")) {
                    var oldSession = new TraceEventSession("IDPSRealTimeSession");
                    oldSession.Dispose();
                }

                _session = new TraceEventSession("IDPSRealTimeSession");
                ...
                _session.Source.Dynamic.All += delegate (TraceEvent data) {
                    try {
                        ...
                        if (data.ProviderName.Contains("Kernel-Process") && data.EventName.Contains("Start")) {
                            ...
                            if (cmd.Contains("logman") && (cmd.Contains("stop") || cmd.Contains("delete")) && cmd.Contains("idpsrealtimesession")) {
                                EventQueue.Enqueue(new IDPSEvent { Provider = "TamperGuard", EventName = "ETW_STOP_ATTEMPT", Details = "A process attempted to blind the IDPS ETW Session via Logman." });
                            }
                        }
                            ...
                            if (protectionObj != null) {
                                ...
                                if (protection == 0x40) {
                                    string proc = string.IsNullOrEmpty(data.ProcessName) ? data.ProcessID.ToString() : data.ProcessName;
                                        EventQueue.Enqueue(new IDPSEvent { Provider = "TamperGuard", EventName = "MEMORY_PATCH_DETECTED", Details = "Suspicious RWX permission change detected in process: " + proc });                              }
                            }
                        }
                            ...
                            if (data.EventName.Contains("Start")) {
                                ...
                                if (isWebParent || isDbParent) {
                                    ...
                                    if (isInterpreter || isSuspiciousPath) {
                                        ...
                                        EventQueue.Enqueue(new IDPSEvent { Provider = "AppGuard", EventName = eventType, Parent = parentContext, Child = childClean, Trigger = trigger, CommandLine = childCmdLine });
                                    }
                                }
                            }
                        }
                            ...
                            try {
                                ...
                                if (payloadLength > 5) {
                                    ...
                                    if (!string.IsNullOrEmpty(ja3Hash)) {
                                        ...
                                        EventQueue.Enqueue(new IDPSEvent { Provider = "NDIS", EventName = "TLS_JA3_FINGERPRINT", DestIp = ndisDestIp, Port = destPort.ToString(), JA3 = ja3Hash });
                                    }
                                }
                            } catch {
                            }
                            ...
                            if (fileName.Contains("\\device\\namedpipe\\") || fileName.Contains("\\pipe\\")) {
                                foreach (string pipePattern in MaliciousPipes) {
                                    if (fileName.Contains(pipePattern)) {
                                        ...
                                        EventQueue.Enqueue(new IDPSEvent { Provider = "P2P_Guard", EventName = "MALICIOUS_PIPE_CREATED", Image = pipeProc, CommandLine = fileName, SuspiciousFlags = "Known C2 Named Pipe" });
                                        break;
                                    }
                                }
                            }
                        ...
                        destIp = ""; string srcIp = ""; string port = ""; string srcPort = ""; string query = ""; string size = "0";
                        ...
                        for (int i = 0; i < data.PayloadNames.Length; i++) {
                            ...
                            if (name == "destinationip" || name == "daddr" || name == "destaddress" || name == "destination" || name == "remoteaddress") {
                                string parsedIp = ParseIp(pVal);
                                if (!string.IsNullOrEmpty(parsedIp) && !parsedIp.Contains("EXCEPTION")) { destIp = parsedIp; }
                                continue;
                            }

                            if (name == "sourceip" || name == "saddr" || name == "sourceaddress" || name == "source" || name == "localaddress") {
                                string parsedIp = ParseIp(pVal);
                                if (!string.IsNullOrEmpty(parsedIp) && !parsedIp.Contains("EXCEPTION")) { srcIp = parsedIp; }
                                continue;
                            }
                            ...
                            if (name == "queryname" || name == "query") query = pStr;
                                ...
                                if (int.TryParse(pStr, out rp)) {
                                    ...
                                    string finalPort = (swapped == 80 || swapped == 443 || swapped == 8080 || swapped == 8443) ? swapped.ToString() : ((swapped < rp && swapped > 0) ? swapped.ToString() : rp.ToString());

                                    if (name.Contains("source") || name.Contains("sport")) srcPort = finalPort;
                                    else port = finalPort;
                                } else {
                                    if (name.Contains("source") || name.Contains("sport")) srcPort = pStr;
                                    else port = pStr;
                                }
                            }
                        }
                        ...
                        string trafficDirection = "Unknown";

                        if (isNetworkEvent) {
                            if ((destIp.StartsWith("127.") && destIp != "127.0.0.99") || destIp.StartsWith("169.254.") || destIp.StartsWith("224.") || destIp.StartsWith("239.") || destIp.StartsWith("fe80") || destIp == "::1" || destIp == "DECODER_FAILED") {
                                return;
                            }

                            bool isDestPrivate = destIp.StartsWith("192.168.") || destIp.StartsWith("10.") || destIp.StartsWith("172.");
                            bool isSrcPrivate = !string.IsNullOrEmpty(srcIp) && (srcIp.StartsWith("192.168.") || srcIp.StartsWith("10.") || srcIp.StartsWith("172."));

                            if (data.ProviderName.Contains("DNS")) {
                                trafficDirection = "Egress";
                            }
                            else if (data.EventName.Contains("Recv") || data.EventName.Contains("Accept")) {
                                trafficDirection = "Ingress";
                            }
                            else if (data.EventName.Contains("Send") || data.EventName.Contains("Connect") || data.EventName.Contains("RemoteEndpoint")) {
                                trafficDirection = "Egress";
                            }
                            else {
                                if (isSrcPrivate && !isDestPrivate) trafficDirection = "Egress";
                                else if (!isSrcPrivate && isDestPrivate) trafficDirection = "Ingress";
                                else if (isSrcPrivate && isDestPrivate) trafficDirection = "Lateral";
                                else if (!string.IsNullOrEmpty(destIp) && !isDestPrivate) trafficDirection = "Egress";
                            }

                            if (trafficDirection == "Lateral" && !(port == "445" || port == "135" || port == "5985" || port == "5986" || port == "3389")) {
                                return;
                            }
                        }
                        ...
                        string safeSrcIp = EscapeJson(srcIp ?? "");
                        string json = null;

                        json = string.Format(@"{{""Provider"":""{0}"",""EventName"":""{1}"",""TimeStamp"":""{2}"",""SrcIp"":""{3}"",""DestIp"":""{4}"",""Port"":""{5}"",""Query"":""{6}"",""Image"":""{7}"",""CommandLine"":""{8}"",""PID"":""{9}"",""TID"":""{10}"",""Size"":""{11}"",""ThreatIntel"":""{12}"",""TrafficDirection"":""{13}""}}",
                            safeProvider, safeEventName, timeStampStr, safeSrcIp, safeDestIp, safePort, safeQuery, safeImage, safeCmdLine, pid, tid, size, safeThreatIntel, safeTrafficDirection);

                        EventQueue.Enqueue(new IDPSEvent {
                        ...
                        try {
                            EventQueue.Enqueue(new IDPSEvent { Provider = "DiagLog", Message = "[ETW HOTPATH ERROR] " + EscapeJson(ex.Message) });
                        } catch { /* prevent recursive failure if enqueue itself fails */ }
                    }
                };
                _session.Source.Process();
            } catch (Exception ex) {
                EventQueue.Enqueue(new IDPSEvent { Error = EscapeJson(ex.Message) });
            }
        });
    ...
    public static void StopSession() {
        ...
        if (_mlEnginePtr != IntPtr.Zero) {
            ...
            EventQueue.Enqueue(new IDPSEvent { Provider = "DiagLog", Message = "[ML ENGINE] Native Rust DLL safely unloaded and DB flushed." });
        }
    }
