<#
.SYNOPSIS
    IDPS Sensor - Rust Engine Compiler Pipeline

.DESCRIPTION
    Compiles the native Rust Machine Learning engine into a C-Compatible Dynamic
    Link Library (idpssensor_ml.dll) to support bidirectional volumetric telemetry.
.AUTHOR
    Robert Weber
#>
...
$WorkingDir = $PWD.Path
$ProjectName = "idpssensor_ml"
$ProjectDir = Join-Path $WorkingDir $ProjectName
$FinalBinaryName = "idpssensor_ml.dll"

$ESC = [char]27
$cRed = "$ESC[91m"; $cCyan = "$ESC[96m"; $cGreen = "$ESC[92m"; $cYellow = "$ESC[93m"; $cReset = "$ESC[0m"

Write-Host "`n$cCyan[*] INITIATING IDPS SENSOR RUST COMPILER PIPELINE (v1.0)$cReset"
...
# ============================================================================
$CompiledDll = Join-Path $ProjectDir "target\release\idpssensor_ml.dll"
...
if (Test-Path $CompiledDll) {
    ...
    Write-Host "$cCyan║$cReset $cGreen SUCCESS: Native IDPS Engine Compiled Successfully"
