...
public class DeepVisibilitySensor {
    ...
    // Match the Rust #[repr(C, packed)] MonitorEvent struct
    [StructLayout(LayoutKind.Sequential, Pack = 1)]
    public struct KernelMonitorEvent {
        public uint event_type;
        public IntPtr pid;
        public IntPtr parent_pid;
        public long timestamp;
        public ushort path_len;
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 260)]
        public char[] path;
        public uint anomaly_score_fixed;
        public uint syscall_id;
        public ulong event_id;
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        public byte[] hash;
        public Guid activity_id;
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 64)]
        public byte[] cloud_context;
        public uint valid;
    }
    ...
    private const uint IOCTL_GET_EVENTS = 0x80002004;

    public static void StartRustDriverIpc() {
        Task.Run(() => {
            IntPtr hDevice = CreateFile(@"\\.\EndpointMonitor", GENERIC_READ | GENERIC_WRITE, 0, IntPtr.Zero, OPEN_EXISTING, 0, IntPtr.Zero);

            if (hDevice == new IntPtr(-1)) {
                EnqueueDiag("[IPC ERROR] Failed to connect to Rust Ring-0 Driver. Are you running as SYSTEM/Admin with TestSigning?");
                return;
            }

            EnqueueDiag("[IPC] Successfully connected to Rust Ring-0 Driver. Awaiting telemetry...");

            int eventSize = Marshal.SizeOf(typeof(KernelMonitorEvent));
            // Fetch up to 100 events per inverted call batch
            int bufferSize = eventSize * 100;
            IntPtr outBuffer = Marshal.AllocHGlobal(bufferSize);

            try {
                while (true) {
                    // This call will pend in the kernel until Rust calls complete_pending_irp()
                    if (DeviceIoControl(hDevice, IOCTL_GET_EVENTS, IntPtr.Zero, 0, outBuffer, (uint)bufferSize, out uint bytesReturned, IntPtr.Zero)) {

                        int eventsRead = (int)bytesReturned / eventSize;
                        for (int i = 0; i < eventsRead; i++) {
                            IntPtr currentPtr = new IntPtr(outBuffer.ToInt64() + (i * eventSize));
                            var kernelEvt = (KernelMonitorEvent)Marshal.PtrToStructure(currentPtr, typeof(KernelMonitorEvent));

                            // Parse the path correctly based on path_len
                            string pathStr = new string(kernelEvt.path, 0, kernelEvt.path_len);

                            // Route the synchronous kernel event into the existing Triage queue
                            EnqueueRaw($"Ring0_Sync_{kernelEvt.event_type}", GetProcessName(kernelEvt.pid.ToInt32()), "", pathStr, "", kernelEvt.pid.ToInt32(), 0);
                        }
                    } else {
                        Thread.Sleep(50); // Fallback throttle if the driver unloads or fails
                    }
                }
            } finally {
    ...
    // --- V3 ACTIVE DEFENSE: RING-0 FEEDBACK LOOP ---
    private const uint IOCTL_QUARANTINE_PID = 0x80002008;

    public static void QuarantinePidInKernel(int targetPid) {
        // Open handle with GENERIC_WRITE to send data down to Ring-0
        IntPtr hDevice = CreateFile(@"\\.\EndpointMonitor", GENERIC_WRITE, 0, IntPtr.Zero, OPEN_EXISTING, 0, IntPtr.Zero);

        if (hDevice == new IntPtr(-1)) {
            EnqueueDiag($"[IPC ERROR] Could not open kernel driver to contain PID {targetPid}. Driver may be unloaded.");
            return;
        }
        ...
        // Write the target PID to unmanaged memory
        Marshal.WriteInt32(inBuffer, targetPid);

        try {
            // Dispatch the write request to the Rust ioctl_handler
            bool success = DeviceIoControl(hDevice, IOCTL_QUARANTINE_PID, inBuffer, (uint)bufferSize, IntPtr.Zero, 0, out uint bytesReturned, IntPtr.Zero);

            if (success) {
                EnqueueDiag($"[RING-0 CONTAINMENT] Successfully registered PID {targetPid} with kernel Minifilter.");
            } else {
                int err = Marshal.GetLastWin32Error();
                EnqueueDiag($"[RING-0 CONTAINMENT ERROR] Failed to route PID {targetPid} to kernel. Win32 Error: {err}");
            }
        } finally {
