...
Design Notes:
Restricting the BeaconML.py daemon to a single thread is a strategic engineering choice designed
to protect the integrity of the real-time C2 detection pipeline. By preventing underlying
C-libraries from spawning multiple threads, the system avoids "thread-bombing" the CPU, which
ensures that the high-priority C# ETW sensor is never starved of the cycles needed to capture
kernel-level telemetry. Furthermore, this approach eliminates potential deadlocks within the piped
Inter-Process Communication (IPC) and avoids the coordination overhead typical of multithreading
when processing the relatively small-batch matrices used for beaconing analysis. Ultimately, this
constraint provides the deterministic, low-jitter performance necessary for a stable and
predictable security-critical architecture.

***Encountered during testing - IPC deadlocks, when multi-threading is attempted.***
"""
