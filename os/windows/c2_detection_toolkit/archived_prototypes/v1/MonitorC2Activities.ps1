                ...
                switch ($event.Id) {
                    ...
                    3 {
                        ...
                        if ($props['IsOutbound']) {
                            ...
                            if (-not $connectionHistory.ContainsKey($key)) { $connectionHistory[$key] = New-Object System.Collections.ArrayList }
                            [void]$connectionHistory[$key].Add($props['Timestamp'])
                            $connectionHistory[$key] = $connectionHistory[$key] | Where-Object { $_ -gt $now.AddMinutes(-$BeaconWindowMinutes) }
                            if (-not $connectionVolume.ContainsKey($key)) { $connectionVolume[$key] = 0 }
                            ...
                            if ($connectionHistory[$key].Count -ge $MinConnectionsForBeacon) {
                                ...
                                for ($i = 1; $i -lt $times.Count; $i++) { [void]$intervals.Add(($times[$i] - $times[$i-1]).TotalSeconds) }
                                if ($intervals.Count -gt 0) {
                                    ...
                                    if ($stdDev -lt $MaxIntervalVarianceSeconds) {
                                        $props.SuspiciousFlags += "Beaconing Anomaly (StdDev: $($stdDev.ToString('N2')) seconds)"
                                        $props.ATTCKMappings += "TA0011: T1071 (Application Layer Protocol); T1571 (Non-Standard Port)"
                                    }
                                }
                            }
                            if ($connectionVolume[$key] -gt $VolumeThreshold) {
                                $props.SuspiciousFlags += "High Volume Anomaly (Count: $($connectionVolume[$key]))"
