...
.DESCRIPTION
    ...
    Architecture Flow:
      ...
      6. Thread-Level Tracking (TID): Extracts Native Thread IDs to isolate injected payloads and lay
         the groundwork for surgical containment.
      7. Anti-Tamper Watchdog: Generates a synthetic DNS heartbeat every 60 seconds. A state-aware
         Deadman's Switch triggers a critical alarm upon ETW blinding and cleanly logs recoveries.
      8. Unified Active Defense: Natively processes ML and Static alerts directly in RAM to autonomously
         terminate processes and isolate IPs at the firewall instantly, eliminating disk-read latency.
      9. Enterprise 24/7 Deployment: Operates continuously with a mathematically pinned, non-scrolling
...
.NOTES
    ...
    .\MonitorKernelC2BeaconHunter_v5.ps1 -EnableDiagnostics

    To enable autonomous Active Defense (process termination & firewall blocking), Run:
    .\MonitorKernelC2BeaconHunter_v5.ps1 -ArmedMode
#>
#Requires -RunAsAdministrator

# ====================== CONFIGURATION & PARAMETERS ======================
param (
    [string]$OutputPath = "C:\Temp\C2KernelMonitoring_v5.jsonl",
...
function Draw-MonitorDashboard([int]$Events, [int]$Flows, [int]$MlSent, [int]$MlEval, [int]$Alerts, [string]$Tamper, [string]$MlHealth, [string]$SysGuard, [int]$Mitigations) {
    ...
    $TitlePlain = "  ⚡ C2 HUNTER V5 | KERNEL MONITOR DASHBOARD"
    ...
    # --- RENDER DASHBOARD ---
    Write-Host "$cCyan╔════════════════════════════════════════════════════════════════════════════════════════════════════╗$cReset"
    Write-Host "$cCyan║$cReset  $cRed⚡ C2 HUNTER V5$cReset | KERNEL MONITOR DASHBOARD$PadTitle$cCyan║$cReset"
    Write-Host "$cCyan╠════════════════════════════════════════════════════════════════════════════════════════════════════╣$cReset"
...
Write-Diag "=== C2 HUNTER V5 DIAGNOSTIC LOG INITIALIZED ===" "INFO"
...
Add-Type -TypeDefinition $CSharpCode -ReferencedAssemblies $RefAssemblies
...
Write-Host "Starting Real-Time ETW Session (No Disk IO)..." -ForegroundColor Yellow
                        ...
                        if ($pyResponse) {
                            ...
                            foreach ($alertKey in $mlResults.PSObject.Properties.Name) {
                                ...
                                if ($alertData.alert) {
                                    ...
                                    # Extract PID from the V5 thread key
                                    $pidVal = "Unknown"
