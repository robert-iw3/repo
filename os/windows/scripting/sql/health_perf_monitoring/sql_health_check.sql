-- SQL Script for Comprehensive SQL Server Health Check: Agent Jobs, Backups, and Enterprise Monitoring
-- This script gathers extensive metrics on SQL Server health, performance, agent jobs, backups,
-- and potential issues for an always-available production database.
...
-- SET XACT_ABORT ON for immediate abort on errors; wrapped sections in TRY...CATCH for better error handling with logging;
...
-- sqlcmd -S servername -d master -E -i this_script.sql -t 300 > /path/to/sql_metrics.prom
-- Use -t for query timeout (e.g., 300 seconds) to cancel if taking too long
...
BEGIN TRY
    ...
    -- Helper: Unix timestamp in milliseconds (for Prometheus)
