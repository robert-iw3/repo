...
- Import the ADMX Policy definitions from this into *C:\windows\PolicyDefinitions* on the system you're trying to modify.
...
```powershell
.\secure-standalone-server.ps1 -firefox $false
```

You can also specify multiple parameters in the command like this:

```powershell
.\secure-standalone-server.ps1 -firefox $false -chrome $false
```
