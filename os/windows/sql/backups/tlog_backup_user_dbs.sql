-- Routine to perform transaction log backups of all user databases
-- @RW

USE [YourMaintenanceDB];
GO

CREATE OR ALTER PROCEDURE dbo.TranLogUserBackup
    @Directory      nvarchar(512)   = N'C:\Backup\LOG',  -- ← Replace with your backup directory
...
BEGIN
    ...
    IF @Debug = 1
        RAISERROR(N'Transaction log backup started: %s', 0, 1, CONVERT(nvarchar(30), @StartTime, 120)) WITH NOWAIT;
    ...
    BEGIN
        IF @Debug = 1
            RAISERROR(N'→ Starting LOG backup for %s', 0, 1, QUOTENAME(@CurrentDB)) WITH NOWAIT;
        ...
        BEGIN
            DECLARE @File nvarchar(512) =
                ...
                N'_' + CAST(@i AS varchar(10)) + N'.trn';

            INSERT INTO @Files (FilePath) VALUES (@File);
            SET @i += 1;
        END;

        SET @Cmd = N'BACKUP LOG ' + QUOTENAME(@CurrentDB) + N' TO ';
        ...
        IF @Debug = 1
            RAISERROR(N'  Executing log backup...', 0, 1) WITH NOWAIT;
        ...
        BEGIN CATCH
            ...
            SET @Msg = N'  LOG backup failed for ' + QUOTENAME(@CurrentDB) + N' → ' + ERROR_MESSAGE();
        ...
        BEGIN
            IF @Debug = 1
                RAISERROR(N'  Verifying log backup...', 0, 1) WITH NOWAIT;
            ...
            BEGIN CATCH
                SET @Error = ERROR_NUMBER();
                SET @Msg = N'  VERIFY failed for LOG ' + QUOTENAME(@CurrentDB) + N' → ' + ERROR_MESSAGE();
                RAISERROR(@Msg, 16, 1) WITH NOWAIT;
            END CATCH
        END

        IF @Debug = 1
            RAISERROR(N'← Finished LOG %s (%s)', 0, 1,
                      QUOTENAME(@CurrentDB),
    ...
    BEGIN
        IF @Debug = 1
            RAISERROR(N'Starting LOG retention cleanup...', 0, 1) WITH NOWAIT;

        SET @CutoffDate = DATEADD(DAY, -@RetentionDays, SYSDATETIME());
        SET @CutoffDateStr = CONVERT(nvarchar(19), @CutoffDate, 126);

        EXEC @ReturnCode = master.dbo.xp_delete_file 0, @Directory, N'trn', @CutoffDateStr;
        IF @ReturnCode <> 0
        BEGIN
            SET @Error = @ReturnCode;
            RAISERROR(N'LOG cleanup failed (code %d)', 16, 1, @ReturnCode) WITH NOWAIT;
        END
    END

    DECLARE @Duration int = DATEDIFF(SECOND, @StartTime, SYSDATETIME());
    SET @Msg = N'Transaction log backup completed in ' + CAST(@Duration AS nvarchar(20)) + N' seconds.';
