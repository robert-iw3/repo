...
DECLARE @OwnerLogin NVARCHAR(128) = N'sa';  -- Owner login
DECLARE @StepName NVARCHAR(128) = N'Run Health Check';  -- Step name
DECLARE @Command NVARCHAR(MAX) = N'sqlcmd -S localhost -d master -E -i "C:\Scripts\sql_health_check.sql" -t 300 > "C:\Monitoring\sql_metrics.prom" 2>&1 & type "C:\Monitoring\sql_metrics.prom" >> "C:\Monitoring\health.log"';  -- Command (tune paths/server/auth)
