...
# --- Core Sensor Payloads & Compile Toolchains ---
Write-Host "[*] Copying Core Sensor Payloads & Rust Source Code..." -ForegroundColor Gray

# Create a source folder for the Rust Driver
$RustSrcDir = Join-Path $StagingDir "DriverSrc"
New-Item -ItemType Directory -Path $RustSrcDir -Force | Out-Null

# Copy Rust source files into the staging directory
$RustFiles = @("Cargo.toml", "build.rs", "lib.rs", "driver.inf", "Makefile.toml")
foreach ($rf in $RustFiles) {
    if (Test-Path $rf) { Copy-Item -Path $rf -Destination $RustSrcDir -Force }
}

$Payloads = @("DeepSensor_Launcher.ps1", "OsSensor.cs", "OsAnomalyML.py", "requirements.txt")
foreach ($p in $Payloads) {
    if (Test-Path $p) { Copy-Item -Path $p -Destination $StagingDir -Force }
}

Write-Host "[*] Downloading Rust, MSVC, SDK, and WDK Toolchains..." -ForegroundColor Gray

# Rust Installer
$RustOut = Join-Path $StagingDir "rustup-init.exe"
Invoke-WebRequest -Uri "https://win.rustup.rs/x86_64" -OutFile $RustOut
Register-FileHash -FilePath $RustOut -LogicalName "Rustup_Installer"

# Visual Studio Build Tools (C++ Linker)
$VsOut = Join-Path $StagingDir "vs_buildtools.exe"
Invoke-WebRequest -Uri "https://aka.ms/vs/17/release/vs_buildtools.exe" -OutFile $VsOut
Register-FileHash -FilePath $VsOut -LogicalName "MSVC_BuildTools"

# Windows 11 SDK
$SdkOut = Join-Path $StagingDir "winsdksetup.exe"
Invoke-WebRequest -Uri "https://go.microsoft.com/fwlink/?linkid=2262345" -OutFile $SdkOut
Register-FileHash -FilePath $SdkOut -LogicalName "WinSDK_Installer"

# Windows 11 WDK
$WdkOut = Join-Path $StagingDir "wdksetup.exe"
Invoke-WebRequest -Uri "https://go.microsoft.com/fwlink/?linkid=2262365" -OutFile $WdkOut
Register-FileHash -FilePath $WdkOut -LogicalName "WinWDK_Installer"
