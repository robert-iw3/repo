...
def update_and_upgrade(distro):
    ...
    logger.info("Updating and upgrading packages (STIG RHEL-08-010120, CIS 1.1.2)")
...
def lock_root_account():
    ...
    logger.info("Locking root account (STIG RHEL-08-010140, CIS 5.4.3)")
    run_command(['passwd', '-l', 'root'])

def set_secure_umask():
    """Set a secure umask (077) in multiple config files (STIG RHEL-08-020024, CIS 5.4.4)."""
    logger.info("Setting secure umask to 077 (STIG RHEL-08-020024, CIS 5.4.4)")
    configs = [
...
def secure_file_permissions():
    """Set secure permissions on sensitive files (STIG RHEL-08-010060, CIS 5.2.2)."""
    logger.info("Securing file permissions (STIG RHEL-08-010060, CIS 5.2.2)")
    files_permissions = {
        '/etc/passwd': 0o644,
        '/etc/group': 0o644,
    ...
    # Remove world-writable permissions (STIG RHEL-08-010070)
    logger.info("Removing world-writable permissions (STIG RHEL-08-010070)")
    run_command("find / -perm -002 -type f -exec chmod o-w {} \;", shell=True, check=False)

def remove_unnecessary_suid_sgid():
    """Remove SUID/SGID bits from common unnecessary binaries (STIG RHEL-08-010149)."""
...
def disable_core_dumps():
    """Disable core dumps via limits.conf and systemd (STIG RHEL-08-010673, CIS 1.6.1)."""
...
def apply_kernel_hardening():
    """Apply kernel hardening via sysctl (STIG RHEL-08-010375, CIS 3.1.2)."""
    logger.info("Applying kernel hardening parameters (STIG RHEL-08-010375, CIS 3.1.2)")
    sysctl_conf = '/etc/sysctl.conf'
    hardening_params = [
        'kernel.kptr_restrict=2',           # STIG RHEL-08-010375
        'kernel.dmesg_restrict=1',          # STIG RHEL-08-010376
        'kernel.printk=3 3 3 3',            # STIG RHEL-08-010377
        'kernel.unprivileged_bpf_disabled=1',# STIG RHEL-08-010378
        'net.core.bpf_jit_harden=2',        # STIG RHEL-08-010379
        'dev.tty.ldisc_autoload=0',         # STIG RHEL-08-010380
        'vm.unprivileged_userfaultfd=0',     # STIG RHEL-08-010381
        'kernel.kexec_load_disabled=1',      # STIG RHEL-08-010382
        'kernel.sysrq=4',                   # STIG RHEL-08-010383
        'kernel.unprivileged_userns_clone=0',# STIG RHEL-08-010384
        'kernel.perf_event_paranoid=3',      # STIG RHEL-08-010385
        'net.ipv4.tcp_syncookies=1',        # STIG RHEL-08-040260
        'net.ipv4.tcp_rfc1337=1',           # STIG RHEL-08-040261
        'net.ipv4.conf.all.rp_filter=1',    # STIG RHEL-08-040262
        'net.ipv4.conf.default.rp_filter=1', # STIG RHEL-08-040263
        'net.ipv4.conf.all.accept_redirects=0', # STIG RHEL-08-040264
        'net.ipv4.conf.default.accept_redirects=0', # STIG RHEL-08-040265
        'net.ipv6.conf.all.accept_redirects=0', # STIG RHEL-08-040266
        'net.ipv6.conf.default.accept_redirects=0', # STIG RHEL-08-040267
        'net.ipv4.conf.all.secure_redirects=0', # STIG RHEL-08-040268
        'net.ipv4.conf.default.secure_redirects=0', # STIG RHEL-08-040269
        'net.ipv4.conf.all.log_martians=1',    # STIG RHEL-08-040270
        'net.ipv4.conf.default.log_martians=1', # STIG RHEL-08-040271
        'net.ipv4.icmp_echo_ignore_broadcasts=1', # STIG RHEL-08-040272
        'net.ipv4.icmp_ignore_bogus_error_responses=1', # STIG RHEL-08-040273
        'net.ipv4.conf.all.accept_source_route=0', # STIG RHEL-08-040274
        'net.ipv4.conf.default.accept_source_route=0', # STIG RHEL-08-040275
        'net.ipv6.conf.all.accept_source_route=0', # STIG RHEL-08-040276
        'net.ipv6.conf.default.accept_source_route=0', # STIG RHEL-08-040277
        'net.ipv4.tcp_timestamps=0',           # STIG RHEL-08-040278
        'fs.suid_dumpable=0',                  # STIG RHEL-08-010673
        'vm.swappiness=1',                     # CIS 3.1.2
    ]
    if os.path.exists(sysctl_conf):
        with open(sysctl_conf, 'a') as f:
            for param in hardening_params:
                f.write(f"{param}\n")
        logger.info(f"Appended kernel parameters to {sysctl_conf}")
        result = run_command(['sysctl', '-p'], check=False)
        if result and result.returncode != 0:
            logger.warning("sysctl -p failed; may be restricted in unprivileged container")
    else:
        logger.warning(f"{sysctl_conf} not found; applying parameters directly")
        for param in hardening_params:
            run_command(['sysctl', '-w', param], check=False)

def configure_password_quality(distro):
    """Configure password quality and policies (STIG RHEL-08-020110, CIS 5.4.1)."""
    logger.info("Configuring password quality and policies (STIG RHEL-08-020110, CIS 5.4.1)")
    if distro in ['ubuntu', 'debian']:
        ...
        logger.info("Alpine: configuring password length in /etc/login.defs")
...
def configure_pam_faillock(distro):
    """Configure pam_faillock for account lockout (STIG RHEL-08-020110)."""
    logger.info("Configuring pam_faillock (STIG RHEL-08-020110)")
...
def configure_pam_pwhistory(distro):
    """Configure pam_pwhistory to remember previous passwords (STIG RHEL-08-020110)."""
    logger.info("Configuring pam_pwhistory (STIG RHEL-08-020110)")
    if distro in ['ubuntu', 'debian', 'rhel']:
        auth_files = ['/etc/pam.d/system-auth', '/etc/pam.d/password-auth']
        for pam_file in auth_files:
            if os.path.exists(pam_file):
...
def remove_unnecessary_packages(distro):
    """Remove unnecessary packages (STIG RHEL-08-010000, CIS 2.2.1)."""
...
def install_required_packages(distro):
    """Install required security packages (STIG RHEL-08-010000)."""
    logger.info("Installing required security packages (STIG RHEL-08-010000)")
    packages = ['crypto-policies', 'iptables', 'rng-tools', 'sudo', 'usbguard']
    if distro in ['ubuntu', 'debian']:
        for pkg in packages:
            result = run_command(['dpkg', '-s', pkg], check=False)
            if result and result.returncode != 0:
...
def configure_auditd(distro):
    """Configure auditd for logging (STIG RHEL-08-030000, CIS 4.1.1)."""
    logger.info("Configuring auditd (STIG RHEL-08-030000, CIS 4.1.1)")
    if distro in ['ubuntu', 'debian']:
        run_command(['apt', 'install', '-y', 'auditd', 'audispd-plugins'], check=False)
        run_command(['systemctl', 'enable', 'auditd'], check=False)
        run_command(['systemctl', 'start', 'auditd'], check=False)
        logger.info("Installed and enabled auditd on Ubuntu/Debian")
    elif distro == 'rhel':
        run_command(['dnf', 'install', '-y', 'audit'], check=False)
        run_command(['systemctl', 'enable', 'auditd'], check=False)
        run_command(['systemctl', 'start', 'auditd'], check=False)
        logger.info("Installed and enabled auditd on rhel")
    elif distro == 'alpine':
        logger.warning("auditd not typically available on Alpine; consider syslog or manual logging")
    else:
        logger.warning("Unknown distro; skipping auditd configuration")

def secure_ssh_config():
    """Secure SSH configuration if present (STIG RHEL-08-010290, CIS 5.2.1)."""
    logger.info("Securing SSH configuration (STIG RHEL-08-010290, CIS 5.2.1)")
    sshd_config = '/etc/ssh/sshd_config'
    if os.path.exists(sshd_config):
        ssh_settings = {
            'PermitRootLogin': 'no',           # STIG RHEL-08-010290
            'Protocol': '2',                   # STIG RHEL-08-010291
            'PermitEmptyPasswords': 'no',      # STIG RHEL-08-010292
            'MaxAuthTries': '4',              # STIG RHEL-08-010293
            'LoginGraceTime': '60',           # STIG RHEL-08-010294
            'X11Forwarding': 'no',            # STIG RHEL-08-010295
            'IgnoreRhosts': 'yes',            # STIG RHEL-08-010296
            'HostbasedAuthentication': 'no',   # STIG RHEL-08-010297
            'PermitUserEnvironment': 'no',     # STIG RHEL-08-010298
        }
        ...
        logger.info("Restarted SSH service")
    openssh_config = '/etc/crypto-policies/back-ends/openssh.config'
    if os.path.exists(openssh_config) or not os.path.exists(openssh_config):
        if not os.path.exists(openssh_config):
            with open(openssh_config, 'a'):
                pass
        replace_or_append(openssh_config, '^Ciphers', 'aes256-ctr,aes192-ctr,aes128-ctr', 'CCE-82888-9')
        replace_or_append(openssh_config, '^MACs', 'hmac-sha2-512,hmac-sha2-256', 'CCE-82888-9')
    opensshserver_config = '/etc/crypto-policies/back-ends/opensshserver.config'
    if os.path.exists(opensshserver_config) or not os.path.exists(opensshserver_config):
        if not os.path.exists(opensshserver_config):
            with open(opensshserver_config, 'a'):
                pass
        replace_or_append(opensshserver_config, '^CRYPTO_POLICY', "'-oCiphers=aes256-ctr,aes192-ctr,aes128-ctr -oMACs=hmac-sha2-512,hmac-sha2-256'", 'CCE-82888-9')
    else:
        logger.warning(f"{sshd_config} not found; skipping SSH configuration")

def configure_login_banner():
    """Configure login banner (STIG RHEL-08-020040)."""
...
def configure_usbguard():
    """Configure USBGuard audit backend (STIG RHEL-08-020040)."""
    logger.info("Configuring USBGuard audit backend (STIG RHEL-08-020040)")
    usbguard_conf = '/etc/usbguard/usbguard-daemon.conf'
    if os.path.exists(usbguard_conf) or not os.path.exists(usbguard_conf):
        if not os.path.exists(usbguard_conf):
            with open(usbguard_conf, 'a'):
                pass
        replace_or_append(usbguard_conf, '^AuditBackend', 'LinuxAudit', 'CCE-82888-9')
    else:
        logger.warning(f"{usbguard_conf} not found; skipping USBGuard configuration")

def configure_sudo():
    """Configure sudo settings (STIG RHEL-08-010384)."""
    logger.info("Configuring sudo settings (STIG RHEL-08-010384)")
    ...
    if os.path.exists(sudoers_file):
        ...
        logger.warning(f"{sudoers_file} not found; skipping sudo configuration")

def configure_pam_login_attempts(distro):
    """Configure PAM to display login attempts (STIG RHEL-08-020110)."""
...
def configure_openssl_entropy():
    """Configure OpenSSL to use strong entropy (STIG RHEL-08-010384)."""
...
def disable_kernel_modules():
    """Disable unnecessary kernel modules (STIG RHEL-08-010384)."""
    logger.info("Disabling unnecessary kernel modules (STIG RHEL-08-010384)")
    blacklist_conf = '/etc/modprobe.d/blacklist.conf'
    modules = ['atm', 'can', 'cramfs', 'firewire-core', 'sctp', 'tipc']
    if not os.path.exists(blacklist_conf):
        with open(blacklist_conf, 'a'):
            pass
    with open(blacklist_conf, 'a') as f:
        for module in modules:
            f.write(f"install {module} /bin/true\n")
            f.write(f"blacklist {module}\n")
            logger.info(f"Blacklisted kernel module {module}")

def remove_empty_passwords(distro):
    """Remove nullok from PAM configurations (STIG RHEL-08-020110)."""
    logger.info("Removing empty passwords (STIG RHEL-08-020110)")
    if distro in ['ubuntu', 'debian', 'rhel']:
        auth_files = ['/etc/pam.d/system-auth', '/etc/pam.d/password-auth']
        for pam_file in auth_files:
            if os.path.exists(pam_file):
...
def configure_dnf_gpgcheck(distro):
    """Configure DNF GPG check for local packages (STIG RHEL-08-010384)."""
    logger.info("Configuring DNF GPG check (STIG RHEL-08-010384)")
    if distro == 'rhel':
        replace_or_append('/etc/dnf/dnf.conf', '^localpkg_gpgcheck', '1', 'CCE-80791-7')
    else:
        logger.warning("DNF GPG check configuration not applicable for non-rhel distros")

def configure_systemd():
    """Configure systemd settings (STIG RHEL-08-010384)."""
    logger.info("Configuring systemd settings (STIG RHEL-08-010384)")
    replace_or_append('/etc/systemd/system.conf', '^CtrlAltDelBurstAction', 'none', 'CCE-80784-2', '%s=%s')

def configure_max_logins(distro):
    """Configure maximum concurrent login sessions (STIG RHEL-08-020110)."""
...
def remove_unnecessary_users_groups():
    """Remove unnecessary users and groups (STIG RHEL-08-020000, CIS 5.4.2)."""
    logger.info("Removing unnecessary users and groups (STIG RHEL-08-020000, CIS 5.4.2)")
    essential_users = ['root', 'bin', 'daemon', 'adm', 'lp', 'sync', 'shutdown', 'halt', 'mail']
...
def main():
    ...
    logger.info(f"Starting hardening process for {distro} container")
    ...
    configure_dnf_gpgcheck(distro)
    configure_systemd()
    configure_max_logins(distro)
    remove_unnecessary_users_groups()

    logger.info("Hardening complete. Review logs for errors. Some changes may require a container restart.")
